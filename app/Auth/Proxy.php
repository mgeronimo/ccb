...
class Proxy {
    ...
    {
        return Proxy::proxy('password', $credentials);
    }

    public static function attemptRefresh()
    {
    ...
    private static function proxy($grantType, array $data = []) 
    {
        try {
            ...
            $client = new Client();
            /*$guzzleResponse = $client->post(sprintf('%s/oauth/access_token', $config->get('app.url')), [
                'form_params' => $data
            ]);*/
            $guzzleResponse = $client->post(sprintf('%s/oauth/access_token', 'http://ccb.app:8000'), [
                'form_params' => $data
            ]);
